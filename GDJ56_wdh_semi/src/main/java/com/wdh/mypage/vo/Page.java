...
public class Page {
	...
	/*
	 * 화면에 그려질 pageNation을 계산하는 클래스 ( pageNo, numPerpage값을 가지고 다님 )
	 */

	private int startPage; // 게시글 화면에 보여질 첫번째 번호
	private int pageEnd; // 게시글 화면에 보여질 마지막 번호
	private boolean prev, next; // 이전버튼, 다음버튼 활성화여부
	
	private int pageNo; // 현재 조회하는 페이지번호
	private int numPerpage = 10; // 화면에 그려질 데이터
	private int totalData; // 전체게시글 수
	
	// 생성자에서는 객체가 생성될때 계산을 처리
	public Page(int pageNo, int numPerpage, int totalData) {
		this.pageNo = pageNo;
...
//		 공식 = (int)Math.ceil(페이지번호 / 페이지네이션개수) * 페이지네이션개수
		this.pageEnd = (int)Math.ceil(this.pageNo * 0.1) * 10;
		
		// 2. startPage결정
		// 공식 = 끝페이지 - 페이지네이션개수 + 1
		this.startPage = this.pageEnd - 10 + 1;
		
		// 3. realEnd(진짜 끝번호) 구해서 endPage의 값을 다시 결정
//		 만약 게시글이 52개라면 -> 진짜 끝번호 6
//		 만약 게시글이 105개라면 -> 진짜 끝번호 11
//		 공식 = (int)Math.ceil(전체게시글수 / 화면에보여질데이터개수)
		int totalPage = (int)Math.ceil(this.totalData / (double)this.numPerpage);
...
//		 11 클릭시 endPage = 14 , realEnd = 14 -> 다음버튼 false
		this.next = this.pageEnd < totalPage;
		
		// 확인
		System.out.println("시작페이지:" + this.startPage + ", 끝페이지:" + this.pageEnd);
